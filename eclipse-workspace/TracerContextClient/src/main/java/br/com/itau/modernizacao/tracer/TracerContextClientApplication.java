...
public class TracerContextClientApplication {
    @Autowired
     ...
     */
    protected <T> T get(Span spanPai, String operationName, URI uri, Class<T> entityClass, RestTemplate restTemplate) {
        Span span = tracer.buildSpan(operationName).asChildOf(spanPai).start();
        try 
        {
